...
namespace FastAndFractured
{
    public class CharacterSpawner
    {
        ...
        private bool PrepareSpawnList(int maxCharactersInGame, string playerCharacterNameCode, int currentPlayers)
        {
            ...
            for (int aiCharacterCount = 0; aiCharacterCount < totalAICharacters; aiCharacterCount++)
            {
                ...
                string aiNameCode = LevelUtilities.GetRandomValueFromList(_availableNameCodesForSpawning, LevelConstants.ERROR_STRING_MESSAGE_CHARACTER_LIST_EMPTY);
        ...
        public bool TrySpawnCharacters(int maxCharactersInGame, string playerCharacterNameCode, int currentPlayers, bool spawnOnlyAIs, out GameObject playerReferenceForAI)
        {
            playerReferenceForAI = null;
            InGameCharacters.Clear();
            InGameCharactersNameCodes.Clear();

            if (!PrepareSpawnList(maxCharactersInGame, spawnOnlyAIs ? "" : playerCharacterNameCode, spawnOnlyAIs ? 0 : currentPlayers))
            {
            ...
            LevelUtilities.ShuffleList(_spawnPoints);
