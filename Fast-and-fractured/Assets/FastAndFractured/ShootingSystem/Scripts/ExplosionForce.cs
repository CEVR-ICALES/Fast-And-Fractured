...
namespace FastAndFractured
{
    public class ExplosionForce : MonoBehaviour
    {
        ...
        [SerializeField] private bool isUsingRaycast = false;
        ...
        private void OnTriggerEnter(Collider other)
        {
            if(other.TryGetComponent(out PhysicsBehaviour otherComponentPhysicsBehaviours))
            {
                ...
                if (isUsingRaycast)
                {
                    Ray ray = new Ray(_explosionCollider.center,other.transform.position);
                    if(Physics.Raycast(ray,out RaycastHit hitInfo, Mathf.Infinity, characterMask))
                    {
                ...
                {
                    contactPoint = _explosionCollider.ClosestPoint(otherPosition);
                }
                Vector3 direction = (contactPoint - _explosionCollider.bounds.center).normalized;
                direction = Vector3.ProjectOnPlane(direction, Vector3.up);
                Vector3 surfacePoint = _explosionCollider.bounds.center + direction * _explosionCollider.bounds.extents.magnitude;
                float distanceToCenter = Vector3.Distance(surfacePoint, _explosionCollider.bounds.center);
                Debug.Log("distanceToCenter : " + distanceToCenter + "ClosestPoint : " + surfacePoint);
